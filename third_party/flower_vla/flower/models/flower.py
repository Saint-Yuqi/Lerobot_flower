...
class FLOWERVLA(pl.LightningModule):
    def __init__(
        ...
        # Vendored patch F5: default action type index used in encode_observations. Upstream
        # hardcodes 1 (eef_delta). Set via attribute, e.g. `model.default_action_type = 3` for SO-101.
        self.default_action_type = 1
    ...
    def encode_observations(self, batch: Dict) -> torch.Tensor:
        ...
        # Vendored patch F3: FreqEmbedder.timestep_embedding expects a 1-D tensor (N,) and
        # internally does t[:, None]. Upstream passed (B, 1, 1) which broke the broadcast and
        # produced (B, 1, 1, dit_dim). Pass (B,) instead so frequency_embeds comes out (B, dit_dim).
        embed_tensor = torch.zeros(len(batch["rgb_obs"]['rgb_static']))
        ...
        return {
            ...
            # Vendored patch F5: use configurable default_action_type instead of hardcoded 1.
            'action_type': torch.full_like(action_type_tensor, self.default_action_type),
            'proprio': proprio,
            'attention_mask': attention_mask,
        }
